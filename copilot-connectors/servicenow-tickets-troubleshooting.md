...
1.  Impersonate the crawling account you created in your ServiceNow instance.
    ...
    Ensure that the account has the following roles: `rest_api_explorer` and `web_service_admin`.
...
The **Only people with access to this data source** option might be unavailable if the service account lacks read permissions to the required tables. Ensure that the account can read the required tables. 
