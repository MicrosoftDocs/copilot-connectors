...
| Authentication | Authorization fails | Verify the account has at least **Full Read** permission at the Web Application level in SharePoint Server. For the indexing account, full control or SharePoint Server farm administrator role is recommended. See [Configure authentication](sharepoint-server-admin-setup.md#configure-authentication). |
