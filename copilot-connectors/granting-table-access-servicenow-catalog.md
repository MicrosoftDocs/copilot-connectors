...
# Grant table access to an account in ServiceNow Catalog

This article explains how to grant table access to a service account in ServiceNow Catalog. The process involves creating a role, assigning it to a user, and configuring row-level and field-level access controls.
...
1. Add the newly created role (`Copilot Connector Account`).
    ...
    > [!NOTE]
    > You might also assign the following roles to the service account so knowledge articles are indexed without any blocking ACL issues: `catalog_admin`, `user_criteria_admin`, `user_admin`. Assigning these roles is optional.
