...
|:------------ |:------------ |:------------ |:------------ |
| Connection settings | The request is malformed or incorrect. | Incorrect PagerDuty REST API URL. | Verify that you're using the correct REST API URL for your service region: `https://api.pagerduty.com` for US or `https://api.eu.pagerduty.com` for EU. For more information, see [Identify the PagerDuty instance URL](pagerduty-schedules-admin-setup.md#identify-the-pagerduty-instance-url). |
...
| Crawl | Partial schedule data indexed. | The connector service account doesn't have access to all schedules. | Verify that the PagerDuty admin account used for authentication has access to all schedules you want to index. If Advanced Permissions is enabled, verify that team assignments are configured correctly. For more information, see [Configure Advanced Permissions](pagerduty-schedules-admin-setup.md#configure-advanced-permissions). |
...
| Permissions | Users don't see schedules they should have access to. | Identity mapping between PagerDuty and Microsoft Entra ID is incorrect. | Verify that PagerDuty user email addresses match Microsoft Entra ID user principal names (UPNs). If they don't match, configure a custom identity mapping. For more information, see [Define PagerDuty attribute mapping](pagerduty-schedules-admin-setup.md#define-pagerduty-attribute-mapping). |
| Permissions | Advanced Permissions not enforced. | Team assignments in PagerDuty are not configured or Advanced Permissions is not enabled. | Verify that Advanced Permissions is enabled in PagerDuty and that schedules are assigned to the appropriate teams. For more information, see [Configure Advanced Permissions](pagerduty-schedules-admin-setup.md#configure-advanced-permissions). |
