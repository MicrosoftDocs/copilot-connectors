...
- Permissions - Lists all groups and individual users associated with the item. If the status is **allow**, all users in those groups can view the item. If the status is **deny**, users in those groups can't access the item. If you configure access permissions as **Allow everyone** instead of **Only people with access** for the connection, everyone can see all items in the index, and no specific permissions are enforced.

- Check user access - Search for a specific user to verify their access to the item. Enter the user name or email address to display a list of groups to which the user belongs. If the user is denied access to any of these groups, their overall permission to view the item is revoked.
...
> - If a user doesn't appear in **Check user access**, it might be due to a failed user mapping or the user not being discovered (for example, the Microsoft Entra ID user ID wasn't found). To resolve this issue, verify that the user has access in the data source and review the user mapping formula and the error reports.
...
| GitHub Cloud Issues | Issue URL | Found in the URL. For example, in `https://github.com/contoso/project/issues/123`, use the complete issue URL. |
| GitHub Cloud Pull Requests | Pull request URL | Found in the URL. For example, in `https://github.com/contoso/project/pull/456`, use the complete pull request URL. |
| GitHub Cloud Knowledge | Repository name, file path | Found in the URL. For example, in `https://github.com/contoso/project/blob/main/docs/README.md`, the repository full name is `contoso/project`, and the file path is `docs/README.md`. |
| GitHub Server Issues | Issue URL | Found in the URL. For example, in `https://github.com/organization/project/issues/123`, use the complete issue URL. |
| GitHub Server Pull Requests | Pull request URL | Found in the URL. For example, in `https://github.com/organization/project/pull/456`, use the complete pull request URL. |
| GitHub Server Knowledge | Knowledge URL | Found in the URL. For example, use the complete URL `https://github.contoso.com/organization/project/blob/main/docs/README.md.` |
...
When you configure an item to be visible to everyone, all users in the organization can access it, regardless of the permissions set in the data source.
...
> - When testing in Microsoft Search or Microsoft 365 Copilot, ensure that you're searching with a searchable or queryable property. For more information, see [Manage schema](/microsoftsearch/manage-search-schema).
