...
1. From the list of available connectors, select **Confluence On-premises**.
...
To roll out to a limited audience, select the toggle next to **Rollout to limited audience** and specify the users and groups to roll the connector out to. For more information, see [Staged rollout for Copilot connectors](/microsoft-365/copilot/connectors/staged-rollout).
...
- For advanced scenarios, you can use a Confluence Query Language (CQL) string to specify conditions for syncing pages. For more information, see [Advanced Searching using CQL](https://developer.atlassian.com/server/confluence/advanced-searching-using-cql/). Note that Microsoft Graph Connector Agent version 3.1.22 or later is required to use this feature.
