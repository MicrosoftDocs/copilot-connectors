...
Partners who want to make a federated connector available in the gallery can submit their remote MCP server to Microsoft for review. For more information, see [Submit a federated connector](submit-mcp.md).
...
Microsoft-published federated connectors are enabled by default for a tenant unless admins disable them. Partner federated connectors must be approved by an admin before they're enabled for the organization. Admins can manage federated Copilot connectors in the Microsoft 365 admin center by choosing **Copilot connectors** > **Your connections**.
