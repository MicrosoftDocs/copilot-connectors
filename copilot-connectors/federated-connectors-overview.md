...
> [!IMPORTANT]
> Admins can enable and disable all default federated connectors in their organization by using a tenant-wide toggle. When you disable the feature, federated connectors aren't automatically enabled in the tenant. The admin must enable each connector individually. For more information, see [Manage federated connectors](manage-federated-connectors.md).
